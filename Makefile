...
#		LIBRR_DIR := $(shell pwd)/intracellular_libs/roadrunner
		LIBRR_DIR := ./addons/libRoadrunner/roadrunner
#		LIBRR_DIR := /home/heiland/dev/roadrunner-2.9.0-manylinux
#		LIBRR_CFLAGS := -I${LIBRR_DIR}/include/rr/C
		LIBRR_CFLAGS := -I${LIBRR_DIR}/include/rr
#		LIBRR_LIBS := ${LIBRR_DIR}/lib
		LIBRR_LIBS := ${LIBRR_DIR}/lib64
	endif
	ifeq ($(UNAME_S),Darwin)
		OSFLAG += -D OSX
		LIBRR_DIR := ./addons/libRoadrunner/roadrunner
...
all: main.cpp $(ALL_OBJECTS)
	...
	@echo Remember to do this before trying to run the executable:
	@echo export LD_LIBRARY_PATH=./addons/libRoadrunner/roadrunner/lib64
	@echo
